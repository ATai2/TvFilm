...
/**
 * 文 件 名: RabDirectorAdapter
 * 创 建 人: Administrator
 * 创建日期: 2016/9/29 10:54
 * 文件描述：
 * 邮   箱:
 * 博   客:
 * 修改时间：
 * 修改备注：
 */
public class RabDirectorAdapter extends BaseAdapter {

    List<DirectListBean.DataBean.DirectorBean> mList;
    Context mContext;
    CateListFilmPresenterImpl mPresenter;

    public RabDirectorAdapter(List<DirectListBean.DataBean.DirectorBean> list, Context context, CateListFilmPresenterImpl presenter) {
        mList = list;
        mContext = context;
        mPresenter = presenter;
    }

    public RabDirectorAdapter(List<DirectListBean.DataBean.DirectorBean> list, Context context) {
        mList = list;
        mContext = context;
    }
